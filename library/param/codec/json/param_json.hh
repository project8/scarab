...
namespace scarab
{
    ...
    bool param_output_json::write_param( const param& a_to_write, XWriter* a_writer )
    {
        ...
        WARN( dlog_param_json, "parameter not written: <" << a_to_write << ">" );
        return false;
    }
    template< class XWriter >
    bool param_output_json::write_param_null( const param& /*a_to_write*/, XWriter* a_writer )
    {
        //WARN( dlog_param_json, "writing null" );
        a_writer->Null();
        return true;
    }
    template< class XWriter >
    ...
    {
        //WARN( dlog_param_json, "writing value" );
        if( a_to_write.is_string() )
        {
            a_writer->String( a_to_write.as_string().c_str() );
            //WARN( dlog_param_json, "writing string to json: " << a_to_write.as_string() );
        }
        else if( a_to_write.is_bool() )
        {
            a_writer->Bool( a_to_write.as_bool() );
            //WARN( dlog_param_json, "writing bool to json: " << a_to_write.as_bool() );
        }
        else if( a_to_write.is_int() )
        {
            a_writer->Int64( a_to_write.as_int() );
            //WARN( dlog_param_json, "writing int to json: " << a_to_write.as_int() );
        }
        else if( a_to_write.is_uint() )
        {
            a_writer->Uint64( a_to_write.as_uint() );
            //WARN( dlog_param_json, "writing uint to json: " << a_to_write.as_uint() );
        }
        else if( a_to_write.is_double() )
        {
            a_writer->Double( a_to_write.as_double() );
            //WARN( dlog_param_json, "writing double to json: " << a_to_write.as_double() );
        }
        return true;
    }
    template< class XWriter >
    ...
    {
        //WARN( dlog_param_json, "writing node" );
        ...
        for( param_node::const_iterator it = a_to_write.begin(); it != a_to_write.end(); ++it )
        {
            ...
            {
                ERROR( dlog_param_json, "Error while writing parameter node" );
                return false;
            }
        }
        a_writer->EndObject();
